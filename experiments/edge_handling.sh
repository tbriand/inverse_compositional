...
if [ "$#" -lt "3" ]; then
    ...
    echo "usage:\n\t$0 in number displacement_max_corner noise_level"
    echo "example toto.tiff 2 5 1" 
    exit 1
fi
...
for i in `seq 2 $NUMBER`; do
    ...
    echo "`compute rmse $centered $FIELDi` (image $i)"
    echo "`compute max $centered $FIELDi` (image $i)"
    # truth vs ICA EDGE
